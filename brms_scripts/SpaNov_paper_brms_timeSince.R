# Script to run the brms model for the SpaNov paper (timeSince)
# Date:  08/07/2025
...
# /* 
# ----------------------------- OLM_7T timeSince ~ time since onset for Run 1 & Run 2 ---------------------------
# */
# Set brms priors
prior_timeSince  <- c(prior(normal(0, 1), class = "Intercept"),
...
## Fit brms model
m_timeSince_run1 <- brm(bf(lastVisit  ~ s_onset_rel  + (s_onset_rel | subject)), 
          data = m_timeSince_run1_data,
          family = lognormal(), 
          prior = prior_timeSince, 
          iter = iter,
          chains = chains,
          cores = cores,
          seed = 1125,
...
## Fit brms model
m_timeSince_run2 <- brm(bf(lastVisit  ~ s_onset_rel  + (s_onset_rel | subject)), 
                           data = m_timeSince_run2_data,
                           family = lognormal(), 
                           prior = prior_timeSince, 
                           iter = iter,
                           chains = chains,
...
summary(m_timeSince_run2)

# Save the model
save(m_timeSince_run1, m_timeSince_run2, file = "fitted_brms_models/SpaNov_m_timeSince.Rdata")
