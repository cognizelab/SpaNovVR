# Script to run the brms model for the SpaNov paper (visits)
# Date:  08/07/2025
...
# /* 
# ----------------------------- OLM_7T visits ~ time since onset for Run 1 & Run 2 ---------------------------
# */
# Set brms priors
prior_visits  <- c(prior(normal(0, 1), class = "Intercept"),
                   prior(normal(0, 1), class = "b"))

# Run 1 
## Remove NA and get correct run
## Use unique name for data frame because of marginaleffects issues
m_visits_run1_data <- na.omit(data_sub[data_sub$run == 1, ])

## Scale x and y
m_visits_run1_data$s_onset_rel  <- scale_m0_sd0.5(m_visits_run1_data$onset_rel)
# Use drop() to deal with marginal effects issues

## Fit brms model
m_visits_run1 <- brm(bf(visits  ~ s_onset_rel  + (s_onset_rel | subject)), 
          data = m_visits_run1_data,
          family = poisson(), 
          prior = prior_visits, 
          iter = iter,
          chains = chains,
...
summary(m_visits_run1)

# Run 2 
## Remove NA and get correct run
## Use unique name for data frame because of marginaleffects issues
m_visits_run2_data <- na.omit(data_sub[data_sub$run == 2, ])

## Scale x and y
m_visits_run2_data$s_onset_rel  <- scale_m0_sd0.5(m_visits_run2_data$onset_rel)
# Use drop() to deal with marginal effects issues

## Fit brms model
m_visits_run2 <- brm(bf(visits  ~ s_onset_rel  + (s_onset_rel | subject)), 
                           data = m_visits_run2_data,
                           family = poisson(), 
                           prior = prior_visits, 
                           iter = iter,
                           chains = chains,
...
summary(m_visits_run2)

# Save the model
save(m_visits_run1, m_visits_run2, file = "fitted_brms_models/SpaNov_m_visits_poisson.Rdata")
