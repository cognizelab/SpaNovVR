...
m_noveltyScore_run1_data <- na.omit(data_sub[data_sub$run == 1, ])

## Scale x and y
m_noveltyScore_run1_data$s_onset_rel    <- scale_m0_sd0.5(m_noveltyScore_run1_data$onset_rel)
m_noveltyScore_run1_data$s_noveltyScore <- drop(scale(m_noveltyScore_run1_data$noveltyScore)) 
# Use drop() to deal with marginal effects issues

## Fit brms model
m_noveltyScore_run1 <- brm(bf(s_noveltyScore  ~ s_onset_rel  + (s_onset_rel | subject)), 
          data = m_noveltyScore_run1_data,
          family = student(), 
...
## Remove NA and get correct run
## Use unique name for data frame because of marginaleffects issues
m_noveltyScore_run2_data  <- na.omit(data_sub[data_sub$run == 2, ])

## Scale x and y
m_noveltyScore_run2_data$s_onset_rel    <- scale_m0_sd0.5(m_noveltyScore_run2_data$onset_rel)
m_noveltyScore_run2_data$s_noveltyScore <- drop(scale(m_noveltyScore_run2_data$noveltyScore))
# Use drop() to deal with marginal effects issues

## Fit brms model
m_noveltyScore_run2 <- brm(bf(s_noveltyScore  ~ s_onset_rel  + (s_onset_rel | subject)), 
                           data = m_noveltyScore_run2_data,
                           family = student(), 
                           prior = prior_noveltyScore, 
                           iter = iter,
                           chains = chains,
...
# Save the model
save(m_noveltyScore_run1, m_noveltyScore_run2, file = "fitted_brms_models/SpaNov_m_noveltyScore.Rdata")
