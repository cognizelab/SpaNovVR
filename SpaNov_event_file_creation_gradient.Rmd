...
# General notes
This scripts creates the event files for the gradient analysis for the SpaNov paper. At the moment, this script only supports data where all trials are completed. This only creates the events for the gradient analysis for the 3, 4, 5 and 6 regressor variant.
...
The naming convention for the gradient analysis is: SpaNov_gradient_xlvl with x for the number of novelty/familiarity levels used in this analysis. The level of smoothing is additionally indicated by adding "_smo6" or "_smo2" add the end. Since the files created here are used with at least two different levels of smoothing, this was done by copying/rennaming the resulting folders.
...
OLMe_7T_SpaNovGradient1 <- paste0(folder, paste0("OLMe_7T_SpaNov_gradient_", rep(noveltyLvl, each = length(subjIDs)), "lvl/"), "sub-", rep(subjIDs, length(noveltyLvl)), "/ses-", sprintf("%02d", ses), "/") 
OLMr_7T_SpaNovGradient1 <- paste0(folder, paste0("OLMr_7T_SpaNov_gradient_", rep(noveltyLvl, each = length(subjIDs)), "lvl/"), "sub-", rep(subjIDs, length(noveltyLvl)), "/ses-", sprintf("%02d", ses), "/") 
```
...
# Run parallel loop 2
dist2closet <- foreach(i = 1:length(subjects), .combine = 'c', .packages = "assortedRFunctions", .export = "objLocations") %dopar% {
  # Subset data to one subject
  subData <- positionData2[positionData2$ppid == subjects[i], ]
...
positionData2$dist2closet <- dist2closet
```

# Prepare the creation of event files by dividing the environment into sectors
Note: The part below is a vestige of previous scripts where multiple sector sizes were used but here only sectoreSize = 10 is used.

```{r sector_table}
# Create the sector table
sector_table <- data.frame(Name = "SpaNov_gradient",
                           sectorSize = 10)
...
numSeeds <- sector_table$sectorSize[1] # Number of values per axis
...
for(subj in 1:length(subjIDs)){
  ...
  periods_agg <- ddply(tempData, c("run", "trial", "period", "sector"), summarise, 
                       ...
                       mean_dist2closet = mean(dist2closet))
  ...
  for(lvl in 1:numLevels){
    for(run in 1:length(runs)){
      ...
      noveltyScore       <- tempData$noveltyScore
      ...
      tempData_agg1 <- na.omit(ddply(tempData, c("lastVisit_gradientLevel"), 
                                     ...
                                     mean_dist2closet = mean(mean_dist2closet)))
...
```{r save_image}
save.image(file = "ignore_eventTable3/images/SpaNov_event_file_gradients.RData")
```
...
### Average distance to the objects
...
p2 <- ggplot(subData_6levels, aes(x = level, y = mean_dist2closet, fill = level)) +
  geom_boxplot() +
  theme(legend.position = "none") +
  labs(title = "Distance to the\nclosest object", 
...
It doesn't seem to be the case that Level 2 is odd relative to the other levels. 
