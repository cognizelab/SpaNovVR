...
for(subj in 1:length(subjIDs)){
  ...
  for(lvl in 1:numLevels){
    for(run in 1:length(runs)){
      ...
      # Subject information
      tempData$ppid <- subjIDs[subj]
      
      # Add to list for later analysis
      event_info[[length(event_info) + 1]] <- tempData
...
```{r save_image}
save.image(file = "event_tables/images/SpaNov_event_file_gradients.RData")
```
